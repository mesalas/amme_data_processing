...
class TestCase(unittest.TestCase):
    def test_read_trades_write_bars(self):
        for symbol in ["ABC", "DEF", "GHI"]:
            test_args = ["test_data/"+symbol +"_NYSE@0_Matching-MatchedOrders.csv.gz",
                         "test_output/"+symbol +"_NYSE@0_daily_bars.csv.gz"]
            make_trade_bars(test_args)

    def test_read_trades_write_intraday_bars(self):
        for symbol in ["ABC", "DEF", "GHI" ]:
            test_args = ["10T",
                         "test_data/" + symbol + "_NYSE@0_Matching-MatchedOrders.csv.gz",
                         "test_output/" + symbol + "_NYSE@0_10T_bars.csv.gz"]
